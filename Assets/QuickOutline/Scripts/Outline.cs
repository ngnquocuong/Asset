...
public class Outline : MonoBehaviour {
  private const string SimplifiedMeshAppendix = "simplified mesh";
  ...
  {
    get => precomputeOutline;
    set => precomputeOutline = value;
  }
#endif
  ...
  void Awake()
  {
    if (ABTestLoader.Instance.GamePlayConfig.highlightObjects)
    {
      Init();  
    }
  ...
  void LoadSmoothNormals() {
    ...
    foreach (var meshFilter in GetComponentsInChildren<MeshFilter>()) {
      ...
      if (meshFilter.sharedMesh != null && meshFilter.sharedMesh.isReadable && smoothNormals.Count > 0)
      {
        if (meshFilter.sharedMesh.vertices.Length == smoothNormals.Count)
        {
          meshFilter.sharedMesh.SetUVs(3, smoothNormals);
        }
        else
        {
          Debug.LogWarning($"[Outline] Mesh {meshFilter.sharedMesh} smooth normals array and vertex array have different sizes."); 
        }
      }
      else
      {
        if (!meshFilter.sharedMesh.name.Contains(SimplifiedMeshAppendix))
        {
          Debug.LogWarning($"[Outline] Mesh {meshFilter.sharedMesh} is not readable, cannot write uv4.");
        }
      }
  ...
  List<Vector3> SmoothNormals(Mesh mesh)
  {
    ...
    if (mesh != null && !mesh.isReadable)
    {
      Debug.LogWarning($"[Outline] Mesh {mesh} is not readable.");
    }
  ...
  void CombineSubmeshes(Mesh mesh, Material[] materials) {
    ...
    // Skip meshes with a single submeshf
